...
void KMP(string gene, int par[], int freq[])
{
    int i, j, k=0;
    par[0]=0; freq[0]=1;
    for(i=1; i<SZ(gene); i++)
    {
        while(k>0 && gene[i]!=gene[k])
            k= par[k-1];
...
//        deb(i, k);
        ...
        par[i]=k;
        deb(i, gene[i], par[i], freq[i]);
    }
    return;
}

void sol(string DNA, string gene, int arr[], int par[])
{
    int i, j, k, cnt, freq[MAX];
    mset(freq, 0);
...
int main()
{
    int i, j, k,x, u, v, w, a, b, c, tcase, t, m, n, cnt=0;
    int arr[MAX], arrRev[MAX], par[MAX], parRev[MAX];
    string gene, DNA, dnaRev;
    ...
    REP(t, tcase)
    {
        ...
        DNA= str1; gene= str2;
//        cin>>DNA>>gene;
        sol(DNA, gene, arr, par);
        reverse(ALL(DNA));
        reverse(ALL(gene));
        sol(DNA, gene, arrRev, parRev);

        ll result=0;
        for(i=0; i<SZ(DNA)-1; i++)
        {
//            deb(i, arr[i], arrRev[(SZ(DNA)-1)-i-1], (SZ(DNA)-1)-i-1);
            result+= (arr[i]*arrRev[(SZ(DNA)-1)-i-1]);
        }
        printf("Case %d: %lld\n",t+1, result);
    }
    return 0;
