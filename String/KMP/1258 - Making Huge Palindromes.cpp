...
int parent[MAX], lenFinal, len;

void KMP(string strFinal)
{
    int i, k;
    CLR(parent);
    parent[0]=k=0;
    lenFinal= SZ(strFinal);

    for(i=1; i<lenFinal; i++)
    {
        while(k>0 && strFinal[k]!=strFinal[i])
            k= parent[k-1];

        if(strFinal[k]==strFinal[i])
            k++;
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int i, j, k, result, t=0, tcase, rest;
    cin>>tcase;
    string str, strFinal;
    while(tcase--)
    {
        cin>>str; len= SZ(str);
        string revrs=str;        reverse(ALL(revrs));
        strFinal = revrs+"#"+str;

        KMP(strFinal);

        rest= ( len - parent[lenFinal-1]);
        result= parent[lenFinal-1]+(rest*2);
        printf("Case %d: %d\n", ++t, result);
    }
}
