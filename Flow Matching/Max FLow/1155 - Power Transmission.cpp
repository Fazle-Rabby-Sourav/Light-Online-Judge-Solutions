...
Problem Link: http://www.lightoj.com/volume_showproblem.php?problem=1155
...
/*
Statement:
DESA is taking a new project to transfer power. Power is generated by the newly established plant in Barisal. The main aim of this project
 is to transfer Power in Dhaka. As Dhaka is a megacity with almost 10 million people DESA wants to transfer maximum amount of power through
 the network. But as always occurs in case of power transmission it is tough to resist loss. So they want to use some regulators whose main
  aims are to divert power through several outlets without any loss.
Each such regulator has different capacity. It means if a regulator gets 100 units of power and its capacity is 80 units then remaining 20 units
 of power will be lost. Moreover each unidirectional link (connectors among regulators) has a certain capacity. A link with capacity 20 units
cannot transfer power more than 20 units. Each regulator can distribute the input power among the outgoing links so that no link capacity is over
flown. DESA wants to know the maximum amount of power which can be transmitted throughout the network so that no power loss occurs. That is the job
you have to do.
*/

/*
solution Method:
It is very beginer level max flow problem. A network of directional edges si given. there is a source node and sink node. each node has a
capacity. it can be implemented by creating another corresponding node and connect them with an edge which weight is equal to the capacity of the
node. then try to send flow from sorce node and itterately check and increament the flow.
when there is no augmenting pathe then the flow is the maximum flow ion this network
*/
...
int num_node,num_edge, source, sink, min_flow, n;
vi edge[MAX];
int cap[MAX][MAX], par[MAX];
...
int main()
{
    int i, j, k, result, t=0, tcase, m, cnt, u, v, w, b, d;
    cin>>tcase;
    while(tcase--){
