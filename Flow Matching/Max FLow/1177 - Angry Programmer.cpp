...
Problem Link: http://www.lightoj.com/volume_showproblem.php?problem=1177
...
The computer of your boss and the central server are in the same network, which is composed of many machines (computers) and wires linking
pairs of those machines. There is at most one wire between any pair of machines and there can be pairs of machines without a wire between them.

To accomplish your objective, you can destroy machines and wires, but you can't destroy neither the computer of your boss, nor the central server,
because those machines are monitored by security cameras. You have estimated the cost of blowing up each machine and the cost of cutting each wire
in the network.

You want to determine the minimum cost of interrupting the communication between your boss' computer and the central server. Two computers A and B
can communicate if there is a sequence of undestroyed machines x1, x2, ..., xn such that x1 = A, xn = B and xi is linked with xi+1 with an uncut wire
(for each 1 ≤ i < n).
*/

/*
Solution Method:
first we define source node and sink node. assume every computer as node. then we connect every computer nodes with other computer nodes as the
statement of the problem. then we siply run Max Flow algo on this network. then we get the amount of maximum flow that can be send at most.
it si the final ans.
*/

/*
Complexity Analysis:
the complexity is O(V*E62);
*/
...
int source, sink, par[MAX], min_flow, num_node, num_edge, lim, cap[MAX][MAX];
vi edge[MAX];
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int i, j, k, result, t=0, tcase, n, m,w,u, v, cnt;
    lim= 55;
    cin>>tcase;
    while(tcase--)
    {
        ...
        for(i=0; i<num_edge; i++)
        {
            ...
            //this is because of bidirectional connection
            cap[u+lim][v] += w;
            cap[v+lim][u] += w;
        }
        result= max_flow();
