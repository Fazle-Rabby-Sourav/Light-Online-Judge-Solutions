...
Problem Link: http://www.lightoj.com/volume_showproblem.php?problem=1403
...
/*
Solution Method:
there are many cities and road between them. we need to find minimum troop number so that every city is reached by only one troop.
In the statement , it is mentioned that there is no cycle. we assume each intersection as a node. and a road between two intersection means
an edge between corresponding nodes. then run DFS to find maximum number of matching. which stands for this fact that number of maximum matching
is the number of nodes which can be reached by a troop landed another node/city.
so the minimum number of troop will be number of city - maximum matchig number.
 */

 /*
 Complexity Analysis:
 The Runnig time of DFS is O(E+V) where E is the number of edges and V is is the number of Vertices. here E= m and V=n.
 so the complexity will be O(n*(n+m)) or approximately O(n^2);
 */
...
int num_node, num_edge, par[MAX], x;
bool col[MAX];
vi edge[MAX];
...
int main()
{
    int i, j, k, result, t=0, tcase, n, m, u, v, cnt;
    cin>>tcase;
    while(tcase--)
    {
