...
//template< class T > inline T _mod(T n, T m){    return (n%m+m)%m;}
...
//compute b^p%m
int Big_mod(ll B,ll P,ll M){ ll R=1; while(P>0)  {if(P%2==1){R=(R*B)%M;}P/=2;B=(B*B)%M;} return (int)R;}
...
string str1, str2;
int dp[MAX][MAX], DP[MAX][MAX], len1, len2, _mod=1000007;

int rec(int now1, int now2)
{
    if(now1>=len1 || now2>=len2)
    {
        return 0;
    }
    int &ret= dp[now1][now2];
    if(ret!=-1) return ret;
    ret= -INF;
    if(str1[now1]==str2[now2])
    {
        ret= max(ret, rec(now1+1, now2+1)+1);
    }
    else
    {
        ret= max(ret, rec(now1+1, now2));
        ret= max(ret, rec(now1, now2+1));
    }
    return ret;
}
...
//    deb(now1, now2);
    ...
    if(str1[now1]==str2[now2] ) //&& dp[now1][now2]== rec(now1+1, now2+1)+1
        RET = (RET+ REC(now1+1,  now2+1)%_mod)%_mod;

    else
    {
//        deb(dp[now1][now2], rec(now1+1, now2), dp[now1][now2], rec(now1, now2+1)+1, "else");
        if(rec(now1, now2) == rec(now1+1, now2))
            RET = (RET+REC(now1+1 , now2)%_mod)%_mod;
        if(rec(now1, now2) == rec(now1, now2+1))
            RET = (RET+REC(now1 , now2+1)%_mod)%_mod;
        if(rec(now1, now2)== rec(now1+1, now2+1))
            RET=  ((RET-(REC(now1+1 , now2+1)%_mod))+_mod) %_mod;
    }
//    deb(now1, now2, RET, "RET");
    return RET;
}

int main()
{
    int i, j, k, u, v, w, x, y, z, t=0, tcase, result=0, maxLen;
    cin>>tcase;
    while(tcase--)
    {
        cin>>str1>>str2;
