...
LL dp[12][83][83], color[12][83][83];
LL MOD;
int t;
LL solution(int now, int sum, int digit)
{
    int i;
    if(now==0)
    {
        if(sum%MOD==0 && digit%MOD==0)  return 1;
        else   return 0;
    }
    LL &ret= dp[now][sum][digit];
    if(color[now][sum][digit]==t)   return ret;

    color[now][sum][digit]= t;    ret=0;
    for(i=0; i<10; i++)
        ret+= solution(now-1, (sum*10+i)%MOD, (digit+i)%MOD);
    return ret;
}
...
LL rec(LL n)
{
    ...
    {
        for(j=1; j<10; j++)
            ans+= solution( (i-1), (j%MOD), (j%MOD) );
    }
    //for MSG
    for(i=1; i<str[0]-'0'; i++)
        ans+= solution(len-1, i%MOD, i%MOD);
    ...
    for(i=1; i<len; i++)
    {
        for(j=0; j<str[i]-'0'; j++)
        {
            ans+= solution(len-i-1, ((sum*10)+j)%MOD, (digi+j)%MOD);
        }
        digi= (digi+(str[i]-'0'))%MOD;
