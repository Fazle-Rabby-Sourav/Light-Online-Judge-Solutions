...
int rec(int now)
{
    ...
    {
        if(now>=item[i].sc.sc)
            ret= max(ret, (rec(now-item[i].sc.sc)+item[i].fs));
    }
    return ret;
}
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int i, j, k, result, t=0, tcase, sum_w, w, p, sum_money;
    cin>>tcase;
    while(tcase--)
    {
        item.clear();
        scanf("%d %d", &n, &w_lim);
        for(i=0, sum_w=0, sum_money; i<n; i++)
        {
            scanf("%d %d %d",&p, &j, &w);
            item.pb(MP(p, MP(j, w)));
            if(j)
