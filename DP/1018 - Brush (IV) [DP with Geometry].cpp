...
int rec(int mask, int now)
{
   // deb(mask, now);
    ...
    if( (mask&(1<<now) )==0)
    {
        ...
        {
            if((mask&(1<<j))==0)
                ret= min(ret, rec( (mask|(in_line[now][j])), now+1)+1 ) ;
        }
    }
    else
        ret= min(ret, rec(mask, now+1));
...
int main()
{
    int i, j, k, result, t=0, tcase, x, y;
    cin>>tcase;
    while(tcase--)
    {
        points.clear(); CLR(in_line);
        scanf("%d", &n);
        for(i=0; i<n; i++)
        {
            scanf("%d %d", &x, &y);
        ...
        for(i=0; i<n; i++)
        {
            in_line[i][i]= (mask|(1<<i));
            for(j=i+1; j<n; j++)
            {
                mask=0;
                for(k=0; k<n; k++)
                {
                    v1=make_vector(points[i], points[k]);
                    v2=make_vector(points[j], points[k]);

                    res= cross_prod(v1, v2);
                    if(res==0)
                    {
                        mask= (mask|(1<<k));
                    }
