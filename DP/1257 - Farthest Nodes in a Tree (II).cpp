...
int num_node, max_dist, farthest_node[2], taken[MAX], dist[2][MAX];
vii edge[MAX];

void dfs(int from, int num_of_dfs)
{
    int i, to;
    taken[from]=1;
    for(i=0; i<SZ(edge[from]); i++)
    {
        ...
        if(!taken[to])
        {
            dist[num_of_dfs][to]= dist[num_of_dfs][from]+edge[from][i].sc;
            if(max_dist<dist[num_of_dfs][to])
            {
                farthest_node[num_of_dfs]= to;
                max_dist= dist[num_of_dfs][to];
            }
            dfs(to, num_of_dfs);
        }
    }
...
int main()
{
    int i, j, k, result, t=0, tcase, a, b, w;
    ...
    while(tcase--)
    {
        ...
        CLR(taken); max_dist=-1;
        dfs(0, 0);

        CLR(dist); CLR(taken); max_dist=-1;
        dfs(farthest_node[0], 0);
        farthest_node[1]= farthest_node[0];

        CLR(taken); max_dist=-1;
        dfs(farthest_node[1], 1);

        printf("Case %d:\n", ++t);
        for(i=0; i<num_node; i++)
        {
            result= max(dist[0][i], dist[1][i]);
            printf("%d\n", result);
        }
