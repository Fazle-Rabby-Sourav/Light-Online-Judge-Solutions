...
int rec(int now, int diff)
{
    if(now==-1)
        return (diff!=0)*(-INF);
    else
        return dp[now&1][diff];


//    int &ret= dp[now][diff];
//    if(ret!=-1) return ret;
//
//    ret= -INF;    ret= max(ret, rec(now-1, diff));
//    if(arr[now]+diff<= sum)
//        ret= max(ret, rec(now-1, diff+arr[now]));
//    if(arr[now]<diff)
//        ret= max(ret, rec(now-1, diff-arr[now])+arr[now] );
//    else
//        ret= max(ret, rec(now-1, arr[now]-diff)+diff );
...
//        sort(ALL(arr) );
        ...
        for(int now=0; now<N; now++)
        {
            ...
            {
                int &ret= dp[now&1][diff];
                ret= -INF;

                ret= max(ret, rec(now-1, diff));
