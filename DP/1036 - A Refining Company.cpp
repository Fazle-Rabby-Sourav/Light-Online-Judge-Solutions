...
int uranium[MAX][MAX], radium[MAX][MAX], row, col;
int dp[MAX][MAX];
int rec(int now, int lim)
{
    int i, k;
    if(now==row)
        return 0;
    ...
    if(ret!=-1) return ret;
    ret= 0;
    int sum_rad=0, sum_uran=0, all_rad=0, all_uran=0;

    for(i=0; i<lim; i++)
        sum_uran+= uranium[now][i];
    for(i=lim; i<col; i++)
        sum_rad+= radium[now][i];

    ret= max(ret, rec(now+1, lim)+sum_rad+sum_uran);
    for(i=lim; i<col; i++)
    {
        sum_uran+= uranium[now][i];
        sum_rad-= radium[now][i];
        ret= max(ret, (rec(now+1, i+1)+sum_uran+sum_rad) );
    }
    return ret;
}
