...
int n, m, indeg[MAX], deg[MAX], dist[MAX][MAX];
        ...
        if((mask&(1<<i))==0)
        {
            for(k=1; k<=n; k++)
            {
                ...
                if((mask&(1<<k))==0)
                {
                    ...
                    newmask = (newmask|(1<<i));
                    newmask =  (newmask|(1<<k));
//                    deb(i, k,mask,  newmask, dist[i][k]);
                    ret= min(ret, rec( newmask )+dist[i][k]);
                }
            }
        }
    }
...
//        deb(cnt, res, dist[1][3], "odd degree nodes");
