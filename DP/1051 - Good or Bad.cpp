...
string str;
int is_good, is_bad, len;
int dp[52][52][52][3];

int rec(int now, int vowel, int conso, int flag)
{
 //   deb(now, vowel, conso, is_ques);
    int next_flag=flag;
    if(vowel>=3 || conso>=5)
    {
        is_bad=1;
    ...
    int &ret= dp[now][vowel][conso][flag];
    if(ret!=-1) return ret;

    ret= 0;
    if(IsVowel(str[now]))
    {
        ret =rec(now+1, vowel+1, 0, next_flag);
    }
    else if(str[now]=='?')
    {
        int left, right;
        left = rec(now+1, vowel+1, 0, next_flag);
        right= rec(now+1, 0, conso+1, next_flag);
    ...
    {
        ret = rec(now+1, 0, conso+1, next_flag);
    }
    return ret;
}
...
int main()
{
    ...
    while(tcase--)
    {
        ...
        printf("Case %d: ", ++t);
        if(result==1 && is_bad==1)
            printf("MIXED\n");
        else if(result==2 && is_bad==1)
            printf("BAD\n");
        else if(is_bad!=1)
            printf("GOOD\n");
    }
