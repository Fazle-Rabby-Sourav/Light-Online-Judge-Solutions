...
struct coin
{
    int val, num;
    coin(){};
    coin (int a, int b){val=a; num=b;}
    bool operator < (const coin &p) const
    {
        return (val*num < p.val*p.num);
    }
};
...
int main()
{
    ...
    while(tcase--)
    {
        ...
        mset(dp, 0);
        dp[0]=1;
        sort(poysa, poysa+num_coin);
        int lim=0;

        for(i=0; i<num_coin; i++)
        {
            lim+= poysa[i].val*poysa[i].num;
            for(j=0; j<=lim && j<=target; j++)
            {
                if(dp[j]>0)
                {
                    if(j+poysa[i].val>target || j+poysa[i].val>lim)
                        break;
                    if(j+poysa[i].val<=target)
                    {
                        dp[j+poysa[i].val]= 1;
                    }
                }
