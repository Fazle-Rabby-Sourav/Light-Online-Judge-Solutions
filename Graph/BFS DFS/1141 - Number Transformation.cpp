...
int prime_series[MAX], Limit, start, dest;
bool is_prime[MAX];
int dp[MAX];
...
void sieve()
{
 ...
 int rec(int now)
 {
    ...
    if(ret!=-1) return ret;
    ret= INF;
    for(i=0; prime_series[i]<=now/2; i++)
    {
        if(now%prime_series[i]==0 && now+prime_series[i]<=dest)
            ret= min(ret, rec(now+prime_series[i])+1);
    }
    return ret;
 }
