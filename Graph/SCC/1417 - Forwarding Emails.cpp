...
int num_node, n_grp_mem, group[MAX], group_mem[MAX], num_group;
int dp[MAX];
bool taken[MAX];
vi edge[MAX], t_edge[MAX], edge_scc[MAX], S;
...
void t_dfs(int from, int grp)
{
    int to, i;
    taken[from]=1;
    group[from]= grp;
    n_grp_mem++;
    for(i=0; i<SZ(t_edge[from]); i++)
    {
        to= t_edge[from][i];
        if(taken[to]==0)
            t_dfs(to, grp);
...
int rec(int from)
{
    ...
    if(ret!=-1) return ret;
    ret=0;
    for(int i=0; i<SZ(edge_scc[from]); i++)
    {
        int to= edge_scc[from][i];
        if(taken[to]==0)
            ret = max(ret, rec(to));
    }
    ret+= group_mem[from];
...
int SCC()
{
    CLR(taken); CLR(group), CLR(group_mem);
    int i, j, k, cnt;
...
//    deb("first okay");
    ...
    for(i=0; i<=num_node; i++)  group_mem[group[i]]++;

    num_group= cnt;
    for(i=0; i<=num_node; i++)  edge_scc[i].clear();
    for(i=1; i<=num_node; i++)
    {
        for(k=0; k<SZ(edge[i]); k++)
        {
            ...
            {
                edge_scc[group[from]].pb(group[to]);
            }
        }
    }

    CLR(taken);     mset(dp, -1);
    int ans, result=-1, ind;
    for(i=1; i<=num_node; i++)
    {
        if(taken[ group[i] ]==0)
            ans= rec(group[i]);
