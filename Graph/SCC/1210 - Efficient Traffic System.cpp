...
int num_node, num_edge, group[MAX], indeg[MAX], outdeg[MAX];
vi edge[MAX], t_edge[MAX], S;

bool taken[MAX];
int dfs(int from)
{
    taken[from]= 1;
    for(int i=0; i<SZ(edge[from]); i++)
    {
        int to;
        to= edge[from][i];
...
int t_dfs(int from, int grp)
{
    group[from]= grp;
    taken[from]=1;
    for(int i=0; i<SZ(t_edge[from]); i++)
    {
        int to;
        to= t_edge[from][i];
...
int SCC()
{
   ...
   // deb(cnt);
    ...
    if (cnt==1)   return 0;
    return max(COME, GO); //here we use max cause one component with indeg=0 can be connected with one component with outdeg=0 by creating one edge. so which one is maximum, we need to create that number of edge
}
