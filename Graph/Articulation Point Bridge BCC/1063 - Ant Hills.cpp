...
#define MAX 10005
int num_node, num_edge, color[MAX], low[MAX], tym[MAX], T;
VI edge[MAX];
set <int> S;
...
void dfs(int from, int par)
{
    ...
    for(i=0; i<SZ(edge[from]); i++)
    {
        ...
        {
            low[from]= min(low[from], tym[to]);
        }

        //if root is an articulation point
        if(par==-1 && subtree>1)
        {
            S.insert(from);
        }
