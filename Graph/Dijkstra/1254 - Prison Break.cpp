...
int dist[MAX][MAX], cap;

int dijkstra(int st, int ed)
{
    int i, j, k, w, to, from, from_cst, to_cst, to_f, from_f;
    priority_queue < edge > pq;
//    deb(st, ed);
    REP(i, MAX) REP(k, MAX) dist[i][k]= INF;

    dist[st][0]= 0;
    pq.push(edge(st, dist[st][0], 0));

    while(!pq.empty())
    {
        from= pq.top().node;
        from_cst= pq.top().cost;
        from_f= pq.top().fuel;
        pq.pop();
        ...
        if(from_cst>dist[from][from_f])
        {
//            deb(from_cst, dist[from][from_f],  "continue");
            continue;
        }
//        deb(from, from_cst, from_f);

        if(from_f+1<=cap && from_cst+fuelCost[from] < dist[from][from_f+1])
        {
            dist[from][from_f+1]= from_cst+fuelCost[from];
            pq.push(edge(from, dist[from][from_f+1], from_f+1));
        }
        ...
        REP(i, SZ(arr[from]))
        {
            ...
            if(from_f>=w && from_cst<dist[to][from_f-w])
            {
                dist[to][from_f-w]= from_cst;
                pq.push(edge(to, from_cst, from_f-w));
            }
        }
    }
    return -1;
...
int main()
{
    int x, i, j, k, result, t=0, tcase, n, m, cnt, u, v, w, st, ed ,q;
