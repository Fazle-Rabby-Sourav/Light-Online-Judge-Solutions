...
int n, m;
bool taken[16][(1<<15)+9], isPossible[(1<<15)+9];

vi adj[16], arrMask;
int dp[(1<<16)+10];

void dfs(int from, int mask)
{
    if(taken[from][mask])   return;
    taken[from][mask]=true;
...
//    deb(mask);
    ...
    while(newmask>0)
    {
        ...
        if(isPossible[newmask])
        {
            ret= min(ret, rec( ((mask)^newmask) )+1);
        }
        newmask--;
    }
    return ret;
