...
//    deb(mask, nowRem);
    ...
    int &ret = dp[mask][nowRem];
    if(ret!=-1) return ret;

    int used[11];    CLR(used);
    ...
    REP(i, N)
    {
        if((mask&(1<<i))==0)
        {
//            if(i>0 && str[i-1]==str[i] && (mask&(1<<(i-1)))!=0 )    continue;
            if(used[(str[i]-'0')]==1) continue;
            used[(str[i]-'0')]=1;
            int nextRem= (((nowRem*10))+(str[i]-'0'))%D;
            ret= (ret+(rec((mask|(1<<i)),  nextRem)));
        }
    }
    return ret;
}
