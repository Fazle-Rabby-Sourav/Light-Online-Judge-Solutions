...
vector <fl> arr;
vi adj[MAX], sol;
int sink, source, flow, penguinN, cap[MAX][MAX], capTemp[MAX][MAX], par[MAX];
...
int main()
{
    ...
    REP(t, tcase)
    {
        arr.clear(); REP(i, N) adj[i].clear(); CLR(cap); CLR(capTemp);
        sol.clear();
        source= 0;
        cin>>N>>D;
        penguinN=0;
