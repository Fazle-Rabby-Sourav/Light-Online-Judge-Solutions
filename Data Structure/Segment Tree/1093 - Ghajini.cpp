...
pii initialize(int node, int left, int right)
{
    if(left==right)
    {
        tree[node].fs= num[left]; tree[node].sc= num[left]; return tree[node];
    }

    pii p, q;

    p= initialize(node*2, left, (left+right)/2 );
    q= initialize(node*2+1 , ((left+right)/2) +1, right);

    int min_val, max_val;
    min_val= min(p.fs, q.fs);
    max_val= max(p.sc, q.sc);

    tree[node]= MP(min_val, max_val);
    return tree[node];
}
...
pii query (int node, int left, int right, int i, int j)
{
    ...
    pii p, q;
    p= query(node*2, left, (left+right)/2, i, j );
    q= query(node*2+1 , ((left+right)/2) +1, right, i, j);

    int min_val, max_val;
    min_val= min(p.fs, q.fs);
    max_val= max(p.sc, q.sc);

    return MP(min_val, max_val);
}
...
int main()
{
    ...
    while(tcase--)
    {
        ...
        for(i=0; i<(N-tym)+1; i++)
        {
            ans= query(1, 0, N-1, i, i+tym-1);
            result = max(result, ((ans.sc) - (ans.fs)) );
        }
        printf("Case %d: %d\n", ++t, result);
    }
}
