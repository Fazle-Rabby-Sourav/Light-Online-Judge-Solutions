...
LL tree[4*MAX], num_updt[4*MAX], num_query, N;
...
void update(int node, int left, int right, int i, int j, int val)
{
    ...
    {
        num_updt[node]+= val;
        tree[node]= tree[node] + (right-left+1)*val;
        return;
    }
    ...
    tree[node]= tree[node*2]+ tree[node*2+1]+ ((right-left+1)*num_updt[node]) ; // if this node was updated before, that value is added
    return;
}
...
LL query(int node, int left, int right, int i, int j)
{
    ...
    LL ans=0;
    int mid= (left+right)/2;
    int ele_num= (min(right, j)- max(left, i) +1);  //to calculate how many elements are in the interval

    ans+= query(node*2, left, mid, i, j);
    ans+= query(node*2+1, mid+1, right, i, j);
    ans+= ele_num*num_updt[node];
    return ans;
}
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int i, j, k, cas, t, command,tcase, b, e, w;
    LL ans;
    cin>>tcase;
    cas=0;
    while(tcase--)
    {
        ...
        for(i=0; i<num_query; i++)
        {
            ...
            {
                scanf("%d %d %d", &b, &e, &w);
                update(1, 0, N-1, b, e, w);
            }
            //print
            else
            {
                scanf("%d %d", &b, &e);
                ans = query(1, 0, N-1, b, e);
                printf("%lld\n", ans);
            }
        }
    }
