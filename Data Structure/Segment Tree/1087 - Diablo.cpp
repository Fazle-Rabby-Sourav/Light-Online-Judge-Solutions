...
int initialize(int node, int left, int right)
{
    ...
    int p, q, mid= (left+right)/2;

    p= initialize(node*2, left, mid);
    q= initialize(node*2 +1, mid+1, right);
...
int query(int node, int left, int right, int indx)
{
    ...
    int x, mid= (left+right)/2;

    if(indx<= tree[node*2])
        x=query(node*2, left, mid, indx);
    else
        x=query( (node*2)+1, mid+1, right, (indx-tree[node*2]) );

    tree[node]= tree[node*2]+tree[node*2 +1];
    return x;
}

int update(int node, int left, int right, int indx)
{
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int cas=0, i, j, k, result, t, tcase, x, pos;
    char command, dum;
    cin>>tcase;
    t=0;
    while(tcase--)
    {
        ...
        initialize(1, 1, N+num_query);
        pos= N;
        printf("Case %d:\n", ++t);
        for(i=0; i<num_query; i++)
        {

            scanf("%c%c",&dum, &command);
            scanf(" %d",&x);

            if(command=='c')
            {
                result= query(1, 1, N+num_query, x);
                if(result!=-1)
                    printf("%d\n", result);
                else
                    printf("none\n");
