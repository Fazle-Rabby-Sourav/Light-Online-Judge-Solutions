...
#define MAX 100005
int tree[4*MAX], scal[4*MAX], N, num_query;
bool upd[4*MAX];

void update(int node, int left, int right, int i, int j, int val)
{
    //if any update is pending
    if(upd[node]==true)
    {
        //if have child
        if(left!=right)
        {
            upd[node*2]=upd[node*2+1]= true;
            scal[node*2]= scal[node*2+1]= scal[node];
        }
        tree[node]= (right-left+1)*scal[node];
        upd[node]=scal[node]=0;
    }
    //outside
    if(left>j || right<i)
        return;
    ...
    if(left>=i && right<=j)
    {
        ...
        {
            upd[node*2]=upd[node*2+1]= true;
            scal[node*2]= scal[node*2+1]= val;      //real updating
        }
        upd[node]=scal[node]= 0;
        return;
    }
    update(node*2, left, (left+right)/2, i, j, val);
    update(node*2+1, (left+right)/2+1, right, i, j, val);
...
int query(int node, int left, int right, int i, int j)
{
    ...
    return query(node*2, left, (left+right)/2, i, j)+query(node*2 + 1, (left+right)/2 +1, right, i, j);
}
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int i, j, k, result, val, ans, t, tcase, selector, a, b, sum, hor;
    scanf("%d", &tcase);
    t=0;
    while(tcase--)
    {
