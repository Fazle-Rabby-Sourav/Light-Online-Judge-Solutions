...
struct data
{
    int val, indx,inp_place, ans;
    bool typ;
    data() {};

    data(int a, int b, bool c, int d)
    {
        val=a; indx= b; typ= c; inp_place= d;
    }
};

vector < data > arr;
int n, num_query;
int tree[4*MAX], result[50005];
int map_indx[MAX];
...
{
   // deb(node, left, right, ind, "update");
    int mid;
    if(left==right)
    {
        tree[node]+= val;
    ...
    mid= (left+right)/2;
    if(ind<=mid)
        update(node*2, left, mid, ind, val);
    else
        update(node*2 +1, mid+1, right, ind, val);
...
int main()
{
    ...
    while(tcase--)
    {
        ...
        for(i=0; i<SZ(arr); i++)
        {
            ...
            {
//                bug(i);
                if(map_indx[arr[i].val]==0)
                {
                    map_indx[arr[i].val]= arr[i].indx;
                    update(1, 1, n, arr[i].indx, 1);
                }
                else
                {
                    int pre_ind, next_ind;
                    pre_ind= map_indx[arr[i].val];
                    next_ind= arr[i].indx;

                    update(1, 1, n, pre_ind, -1);

                    map_indx[arr[i].val]= next_ind;
                    update(1, 1, n, next_ind, 1);
                }
            }
            //querry
            else
            {
                result[ arr[i].inp_place ]= query(1, 1, n, arr[i].val, arr[i].indx);
            }
        }
        printf("Case %d:\n" , ++t);
        for(i=1; i<=num_query; i++)
