...
int N, q, val[MAX], L[MAX], par[MAX][18];
vi tree[MAX];
bool taken[MAX];

void dfs(int from, int p, int lev)
{
    L[from]= lev;
    par[from][0]= p;
    taken[from]=1;
...
int query(int node, int x)
{
    ...
    for (i = log; i >= 0; i--)
    {
//        deb(i, par[node][i], val[par[node][i]], "before if");
            if (par[node][i] != -1 && val[par[node][i]] >=x )
                node = par[node][i];
    }
    return node;
}
...
int main()
{
    //READ("in.txt");
    //WRITE("out.txt");
    int i, j, k, result, t=0, tcase, n, m, cnt, x, v;
