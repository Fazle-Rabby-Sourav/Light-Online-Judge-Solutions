...
int N, tree[4*MAX], mod=1000000007;
...
int query(int node, int st, int ed, int i, int j)
{
    ...
    int p, q;
    p= query(node*2, st, mid, i, j)%mod;
    q= query(node*2 +1, mid+1, ed, i, j)%mod;

    return (p+q)%mod;
}

void update(int node, int st, int ed, int ind, int val)
{
    if(st==ed)
    {
    ...
    if(ind<=mid)
        ...
        update(node*2 +1, mid+1, ed, ind, val);
