...
struct matrix
{
    ...
    matrix operator*(const matrix &B)const
    {
        ...
        REP(i,row) REP(j,B.col) REP(k,col)
            temp.mat[i][j]=(temp.mat[i][j] + (mat[i][k]*B.mat[k][j])%10 )%10;
        return temp;
    }
    matrix operator+(const matrix &B)const
    {
        ...
        REP(i,row) REP(j,col)
            temp.mat[i][j]=(mat[i][j]+B.mat[i][j])%10;
        return temp;
    }
    void print(ll row,ll col)
    {
        ll i,j;
        REP(i,row) {REP(j,col) cout<<mat[i][j]%10;  cout<<endl;}
    }
};
...
{
//    deb(now, "now");
    if(now==1)  return base;
    matrix lft(N, N), rgt(N, N),  ret(N, N);
    if(now&1)
    {
        lft= rec(now-1);
//        ret= lft;
        rgt= MatExpo(base, now);
...
int main()
{
    ...
    ll i, j, k, u, v, w, x, y, z, t=0, tcase, res=0;
    cin>>tcase;
    while(tcase--)
    {
        scanf("%lld %lld", &N, &K);
