...
struct matrix
{
    ...
    matrix operator*(const matrix &B)const
    {
        ...
        REP(i,row) REP(j,B.col) REP(k,col)
            temp.mat[i][j]=(temp.mat[i][j]+mat[i][k]*B.mat[k][j])%MOD;
        return temp;
    }
    matrix operator+(const matrix &B)const
    {
...
int main()
{
    int i, j, k, u, v,n, w, x, y, z, t=0, tcase, result=0;
    matrix M;
    matrix dum;
    M= build_base_matrix();
    ...
    while(tcase--)
    {
        ...
        result= (dum.mat[0][0]+dum.mat[0][3])%MOD; //only these two states are valid for initial case
        printf("Case %d: %d\n", ++t, result);
    }
    return 0;
}
