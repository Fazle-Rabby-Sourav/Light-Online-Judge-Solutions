...
vvi Multiplication(vvi A, vvi B)
{
    int i, j, k, n, m, p;
    n= SZ(A); p= SZ(B); m= SZ(B[0]);
    vvi M(n, vi(m));
    REP(i, n)
        REP(j, m)
            REP(k, p)
...
vvi matExpo(vvi A, int step)
{
    vvi M;
    if(step==1) return A;
    if(step%2==1)   return Multiplication( matExpo(A, step-1), A);
    M= matExpo(A, step/2);
...
int main()
{
    int i, j, k,x, u, v, w, a1, b1, c1, a2, b2, c2, f0, f1, f2, g0, g1, g2, tcase, t, m, n, cnt=0, res1=0, res2=0, query;
...
//        REP(i, 3)   scanf("%d", &g[i]);
        ...
        REP(k, query)
        {
            ...
            {
                mat= matExpo(mat, n-2);
                res1= ( (mat[0][0]*f2)%MOD + (mat[0][1]*f1)%MOD + (mat[0][2]*f0)%MOD + (mat[0][3]*g2)%MOD + (mat[0][4]*g1)%MOD + (mat[0][5]*g0)%MOD )%MOD;
                res2= ( (mat[3][0]*f2)%MOD + (mat[3][1]*f1)%MOD + (mat[3][2]*f0)%MOD + (mat[3][3]*g2)%MOD + (mat[3][4]*g1)%MOD + (mat[3][5]*g0)%MOD )%MOD;
            }
            res1%= MOD;
            res2%= MOD;
            printf("%d %d\n", res1, res2);
